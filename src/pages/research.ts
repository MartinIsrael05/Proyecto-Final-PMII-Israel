...
async function initResearchPage(root: HTMLElement): Promise<void> {
  const links = Array.from(root.querySelectorAll<HTMLAnchorElement>("[data-research-link]"));
...
function renderVueLoadError(): void {
  const demoIds = [
    "vue-reactivity-demo",
...
function mountVueDemos(vue: VueRuntime): void {
    ...
    createApp({
      setup() {
        ...
        const filteredAlbums = computed(() => {
          ...
          if (!normalizedQuery) return albums.value;
  ...
  if (conditionalTarget) {
    createApp({
      ...
      setup() {
        const mode = ref<"if" | "show">("if"); // Cambia entre v-if y v-show (default: v-if)
    ...
    createApp({
      ...
      setup() {
        ...
        const scoreDescription = computed(() => {
          if (score.value <= 2) return "Feedback bajo";
          if (score.value === 3) return "Feedback neutral";
  ...
  if (asyncTarget) {
    createApp({
      setup() {
        ...
        const fakeSearch = async (term: string): Promise<string[]> => {
